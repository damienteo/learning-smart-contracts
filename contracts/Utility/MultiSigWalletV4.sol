...
contract MultiSigWalletV4 {
    ...
    modifier confirmedByAddress(uint256 _txIndex, address _owner) {
        if (!isConfirmed[_txIndex][_owner]) revert TxNotConfirmed();
        _;
    }
    ...
    function confirm(address _owner, uint256 _txIndex)
        private
        txExists(_txIndex)
    ...
    function confirmTransaction(uint256 _txIndex) public onlyOwner {
        confirm(msg.sender, _txIndex);
    }

    // Potential to merge all three OnBehalf methods together:
    // Using 3 bytes32 storage variables: CONFIRM, REVOKE, EXECUTE,
    // with 1 transactOnBehalf method that will take in a additional 'method' parameter,
    // ('method' parameter will decide which method: confirm(), revoke(), execute())
    // However, this could result in additional gas fees (transactOnBehalf will have to run if/else for every such transaction).
    // A pro is that we can also take into account the method for the signature:
    // in addition to nonce, txIndex, and owner address, we can have method as an additional signing input
    // But that is limited upside to this for additional complexity,
    // Since nonce already ensures each signature can be used only once,
    // While _txIndex ensures that the signature is usable only for said transaction
        ...
        validNonceAndSignature(_nonce, _txIndex, _owner, signature)
    {
        ...
        confirm(_owner, _txIndex);
    }

    function revoke(address _owner, uint256 _txIndex)
        private
        txExists(_txIndex)
        notExecuted(_txIndex)
        confirmedByAddress(_txIndex, _owner)
    {
        Transaction storage transaction = transactions[_txIndex];
        ...
        isConfirmed[_txIndex][_owner] = false;

        emit RevokeConfirmation(_owner, _txIndex);
    }

    function revokeConfirmation(uint256 _txIndex) public onlyOwner {
        revoke(msg.sender, _txIndex);
    }

    function revokeConfirmationOnBehalf(
        uint256 _nonce,
        uint256 _txIndex,
        address _owner,
        bytes calldata signature
        ...
        validNonceAndSignature(_nonce, _txIndex, _owner, signature)
    {
        ...
        revoke(_owner, _txIndex);
    }

    function execute(address _owner, uint256 _txIndex)
        private
        txExists(_txIndex)
        notExecuted(_txIndex)
        returns (bytes memory)
    {
        ...
        emit ExecuteTransaction(_owner, _txIndex);
    ...
    function executeTransaction(uint256 _txIndex)
        ...
        returns (bytes memory)
    {
        bytes memory returnData = execute(msg.sender, _txIndex);

        return returnData;
    }

    function executeTransactionOnBehalf(
        uint256 _nonce,
        uint256 _txIndex,
        address _owner,
        bytes calldata signature
        ...
        returns (bytes memory)
    {
        ...
        bytes memory returnData = execute(_owner, _txIndex);
