...
const multiSigWalletJson = require("../../artifacts/contracts/utility/MultiSigWalletV3.sol/MultiSigWalletV3.json");

const MultiSigWalletV3ContractInterface = new ethers.utils.Interface(
  multiSigWalletJson.abi
);

describe("MultiSigWalletV3", () => {
  let owner: SignerWithAddress,
    ...
    MultiSigWalletV3Contract: MultiSigWalletV3,
  ...
  beforeEach(async () => {
    ...
    const MultiSigWalletV3Factory = await ethers.getContractFactory(
      "MultiSigWalletV3"
    );
    MultiSigWalletV3Contract = await MultiSigWalletV3Factory.deploy(
      multiSigOwners,
  ...
  describe("Deployment", async () => {
    it("reverts if zero owner addressed are passed", async () => {
      const nextMultiSigWalletV3Factory = await ethers.getContractFactory(
        "MultiSigWalletV3"
      );

      await expect(
        nextMultiSigWalletV3Factory.deploy([], multiSigOwners.length)
      ).to.be.revertedWithCustomError(
        nextMultiSigWalletV3Factory,
        "OwnersRequired"
      );
    });
    ...
    it("reverts if _numConfirmationsRequired is less than number of owners", async () => {
      ...
      await expect(
        nextMultiSigWalletV3Factory.deploy(
          multiSigOwners,
    ...
    it("reverts if _numConfirmationsRequired is zero", async () => {
      ...
      await expect(
        nextMultiSigWalletV3Factory.deploy(multiSigOwners, 0)
      ).to.be.revertedWithCustomError(
        nextMultiSigWalletV3Factory,
        "InvalidRequiredConfirmations"
      );
    ...
    it("returns the owners of the contract", async () => {
      const owners = await MultiSigWalletV3Contract.getOwners();
    ...
    it("returns the transactionCount of the contract", async () => {
      const transactionCount =
        await MultiSigWalletV3Contract.getTransactionCount();
  ...
  describe("Nonces", async () => {
    it("returns initial nonce of wallet as 0", async () => {
      const nonce = await MultiSigWalletV3Contract.nonces(owner.address);
      expect(nonce).to.equal(0);
    });

    it("returns next nonce of wallet as 1", async () => {
      const nextNonce = await MultiSigWalletV3Contract.getNextNonce();
  ...
  describe("Verification of Signature", async () => {
    ...
    beforeEach(async () => {
      nextNonce = await MultiSigWalletV3Contract.connect(addr2).getNextNonce();

      hash = await MultiSigWalletV3Contract.getMessageHash(
        nextNonce,
        nextTxIndex,
        signerAddress
      );
    ...
    it("verifies the signature", async () => {
      expect(
        await MultiSigWalletV3Contract.verify(
          nextNonce,
          nextTxIndex,
          signerAddress,
          signature
    ...
    it("returns false if wrong signature is used", async () => {
      const nextHash = await MultiSigWalletV3Contract.getMessageHash(
        nextNonce + 1,
        nextTxIndex,
        signerAddress
      );
  ...
  describe("Submit Transaction", async () => {
    it("allows one owner to submit a transaction", async () => {
      ...
      await expect(
        MultiSigWalletV3Contract.connect(addr1).submitTransaction(
          TestContractAddress,
          nextValue,
          data
        )
    ...
    it("stores input data in the contract", async () => {
      ...
      await MultiSigWalletV3Contract.connect(addr1).submitTransaction(
        TestContractAddress,
        nextValue,
        data
      );

      const submittedTx = await MultiSigWalletV3Contract.getTransaction(0);
    ...
    it("has confirmation status as initially false after transaction is newly submitted", async () => {
      const confirmation = await MultiSigWalletV3Contract.isConfirmed(
        txIndex,
        addr1.address
      );
    ...
    it("prevents non-owners from submitting transactions", async () => {
      ...
      await expect(
        MultiSigWalletV3Contract.connect(addr5).submitTransaction(
          TestContractAddress,
          nextValue,
          data
        )
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "NotOwner");
    });
    ...
    it("emits an event upon successful submission of transaction", async () => {
      ...
      await expect(
        ...
        .to.emit(MultiSigWalletV3Contract, "SubmitTransaction")
  ...
  describe("Confirm Transaction", async () => {
    ...
    it("allows one owner to confirm a transaction", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(addr2).confirmTransaction(txIndex)
      ).not.to.be.reverted;
    });

    it("changes confirmation status accordingly after user confirms a transaction", async () => {
      await MultiSigWalletV3Contract.connect(addr2).confirmTransaction(txIndex);
    ...
    it("increases transaction's number of confirmations accordingly after user confirms a transaction", async () => {
      const prevTx = await MultiSigWalletV3Contract.getTransaction(txIndex);
      const prevNumOfConfirmations = prevTx.numConfirmations;

      await MultiSigWalletV3Contract.connect(addr2).confirmTransaction(txIndex);

      const tx = await MultiSigWalletV3Contract.getTransaction(txIndex);
    ...
    it("stores input data in the contract", async () => {
      ...
      const submittedTx = await MultiSigWalletV3Contract.getTransaction(
        txIndex
      );
      expect(submittedTx[4]).to.equal(1); // tx.numConfirmations

      await MultiSigWalletV3Contract.connect(addr3).confirmTransaction(txIndex);

      const submittedTxNextInfo = await MultiSigWalletV3Contract.getTransaction(
        txIndex
      );
    ...
    it("prevents non-owners from confirming transactions", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(addr6).confirmTransaction(txIndex)
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "NotOwner");
    });

    it("prevents confirmation of non-existing transactions", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(addr4).confirmTransaction(txIndex + 1)
      ).to.be.revertedWithCustomError(
        MultiSigWalletV3Contract,
        "TxDoesNotExist"
      );
    ...
    it("emits an event upon successful confirmation of transaction", async () => {
      await expect(
        ...
        .to.emit(MultiSigWalletV3Contract, "ConfirmTransaction")
  ...
  describe("Confirm Transaction on behalf with Signature", async () => {
    ...
    beforeEach(async () => {
      ...
      nextNonce = Number(
        await MultiSigWalletV3Contract.connect(addr2).getNextNonce()
      );

      const hash = await MultiSigWalletV3Contract.getMessageHash(
        nextNonce,
        txIndex,
        addr2.address
      );
    ...
    it("allows one owner to confirm a transaction", async () => {
      await expect(
        MultiSigWalletV3Contract.confirmTransactionOnBehalf(
          nextNonce,
          txIndex,
          signerAddress,
          signature
    ...
    it("changes confirmation status accordingly after user confirms a transaction", async () => {
      const prevConfirmation = await MultiSigWalletV3Contract.isConfirmed(
        txIndex,
        signerAddress
      );
      expect(prevConfirmation).to.equal(false);

      await MultiSigWalletV3Contract.confirmTransactionOnBehalf(
        nextNonce,
        txIndex,
        signerAddress,
        signature
    ...
    it("increases nextNonce for confirmer after user confirms a transaction", async () => {
      const prevNonce = await MultiSigWalletV3Contract.connect(
        addr2
      ).getNextNonce();
      await MultiSigWalletV3Contract.confirmTransactionOnBehalf(
        nextNonce,
      ...
      const newNonce = await MultiSigWalletV3Contract.connect(
        addr2
      ).getNextNonce();
    ...
    it("allows non-owners to confirming transactions on behalf with valid signatures", async () => {
      await expect(
        await MultiSigWalletV3Contract.connect(
          addr6
    ...
    it("prevents confirmation with nonce that is less than nextNonce", async () => {
      ...
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "InvalidNonce");
    });
  ...
  describe("Revoke Transaction", async () => {
    ...
    it("allows one owner to revoke a transaction", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(addr2).revokeConfirmation(txIndex)
      ).not.to.be.reverted;
    });

    it("changes confirmation status accordingly after user revokes a transaction", async () => {
      await MultiSigWalletV3Contract.connect(addr2).revokeConfirmation(txIndex);
    ...
    it("prevents non-owners from revoking transactions", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(addr6).revokeConfirmation(txIndex)
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "NotOwner");
    });

    it("prevents revoking of confirmations from non-exisitng transactions", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(addr2).revokeConfirmation(txIndex + 1)
      ).to.be.revertedWithCustomError(
        MultiSigWalletV3Contract,
        "TxDoesNotExist"
      );
    ...
    it("prevents revoking of confirmations if confirmations had not previously been made", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(addr1).revokeConfirmation(txIndex)
      ).to.be.revertedWithCustomError(
        MultiSigWalletV3Contract,
        "TxNotConfirmed"
      );
    ...
    it("emits an event upon successful confirmation of transaction", async () => {
      await expect(
        ...
        .to.emit(MultiSigWalletV3Contract, "RevokeConfirmation")
        .withArgs(addr2.address, txIndex);
    });
  });
  ...
  describe("Execute Transaction", async () => {
    ...
    beforeEach(async () => {
      ...
      await expect(
        MultiSigWalletV3Contract.connect(addr1).confirmTransaction(txIndex)
      );
      ...
      await expect(
        MultiSigWalletV3Contract.connect(addr3).confirmTransaction(txIndex)
      );
    });
    ...
    it("allows one owner to execute a transaction", async () => {
      ...
      await expect(
        MultiSigWalletV3Contract.connect(owner).executeTransaction(txIndex)
      ).not.to.be.reverted;
    ...
    it("emits an event upon successful execution of transaction", async () => {
      await expect(
        ...
        .to.emit(MultiSigWalletV3Contract, "ExecuteTransaction")
        .withArgs(owner.address, txIndex);
    });

    it("prevents non-owner from executing a transaction", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(addr5).executeTransaction(txIndex)
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "NotOwner");
    });

    it("prevents execution of non-existent transaction", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(owner).executeTransaction(txIndex + 1)
      ).to.be.revertedWithCustomError(
        MultiSigWalletV3Contract,
        "TxDoesNotExist"
      );
    ...
    it("prevents execution of transaction which has already executed", async () => {
      await MultiSigWalletV3Contract.connect(owner).executeTransaction(txIndex);
    ...
    it("prevents confirmation of a transaction which has already executed", async () => {
      ...
      await expect(
        MultiSigWalletV3Contract.connect(addr4).confirmTransaction(txIndex)
      ).to.be.revertedWithCustomError(
        MultiSigWalletV3Contract,
        "TxAlreadyExecuted"
      );
    ...
    it("prevents execution of transaction which does not have enough confirmations", async () => {
      ...
      await expect(
        MultiSigWalletV3Contract.connect(owner).executeTransaction(nextTxIndex)
      ).to.be.revertedWithCustomError(
        MultiSigWalletV3Contract,
      ...
      await expect(
        MultiSigWalletV3Contract.connect(addr1).confirmTransaction(nextTxIndex)
      );
      ...
      await expect(
        MultiSigWalletV3Contract.connect(addr2).confirmTransaction(nextTxIndex)
      );
      ...
      await expect(
        MultiSigWalletV3Contract.connect(addr3).confirmTransaction(nextTxIndex)
      );
  ...
  describe("Deposit and Transfer of Network Token", async () => {
    ...
    it("Updates the balance of the Smart Contract when value is sent", async () => {
      const prevBalance = await ethers.provider.getBalance(
        MultiSigWalletV3Contract.address
      );

      await owner.sendTransaction({
        to: MultiSigWalletV3Contract.address,
        value: sentValue,
      });
    ...
    it("Emits Deposit event when value is transfered to the contract", async () => {
      await expect(
        ...
        .to.emit(MultiSigWalletV3Contract, "Deposit")
    ...
    it("Allows transactions in which value is dispersed to other contracts", async () => {
      ...
      await MultiSigWalletV3Contract.connect(owner).submitTransaction(
        addr1.address,
        sentValue,
        "0x" // the equivalent of empty data
      );

      await MultiSigWalletV3Contract.connect(addr1).confirmTransaction(txIndex);
  ...
  describe("Add Owner", async () => {
    beforeEach(async () => {
      const addOwnerData = MultiSigWalletV3ContractInterface.encodeFunctionData(
        "addOwner",
        [addr5.address]
      );
    ...
    it("updates owners array", async () => {
      const prevOwners = await MultiSigWalletV3Contract.getOwners();

      await MultiSigWalletV3Contract.connect(owner).executeTransaction(txIndex);

      const nextOwners = await MultiSigWalletV3Contract.getOwners();
    ...
    it("updates isOwner status for new address", async () => {
      expect(await MultiSigWalletV3Contract.isOwner(addr5.address)).to.equal(
        false
      );
    ...
    it("emits OwnerAddition event when owner is added", async () => {
      await expect(
        ...
        .to.emit(MultiSigWalletV3Contract, "OwnerAddition")
    ...
    it("does not allow re-addition of existing owner", async () => {
      ...
      await expect(
        MultiSigWalletV3Contract.connect(owner).executeTransaction(nextTxIndex)
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "TxFailure");
    });
    ...
    it("does not allow other wallets to directly call the addOwner method", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(owner).addOwner(addr6.address)
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "NotMultiSig");
    });
  });
  ...
  describe("Remove Owner", async () => {
    beforeEach(async () => {
      const removeOwnerData =
        MultiSigWalletV3ContractInterface.encodeFunctionData("removeOwner", [
          addr3.address,
        ]);
    ...
    it("updates isOwner status for removed address", async () => {
      expect(await MultiSigWalletV3Contract.isOwner(addr3.address)).to.equal(
        true
      );
    ...
    it("emits OwnerRemoval event when owner is removed", async () => {
      await expect(
        ...
        .to.emit(MultiSigWalletV3Contract, "OwnerRemoval")
    ...
    it("automatically reduces numConfirmationsRequired if enough owners are removed ", async () => {
      ...
      await MultiSigWalletV3Contract.connect(addr1).confirmTransaction(
        nextTxIndex
      );

      await MultiSigWalletV3Contract.connect(addr2).confirmTransaction(
        nextTxIndex
      );

      await MultiSigWalletV3Contract.connect(owner).confirmTransaction(
        nextTxIndex
      );

      await MultiSigWalletV3Contract.connect(owner).executeTransaction(
        nextTxIndex
      );
      ...
      await expect(
        MultiSigWalletV3Contract.connect(owner).executeTransaction(
          nextTxIndex + 1
        )
      )
        .to.emit(MultiSigWalletV3Contract, "RequirementChange")
        .withArgs(nextOwnerLength);

      const nextNumConfirmationsRequired =
        await MultiSigWalletV3Contract.numConfirmationsRequired();
    ...
    it("does not allow other wallets to directly call the removeOwner method", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(owner).removeOwner(owner.address)
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "NotMultiSig");
    });
  });
  ...
  describe("Replace Owner", async () => {
    beforeEach(async () => {
      const replaceOwnerData =
        MultiSigWalletV3ContractInterface.encodeFunctionData("replaceOwner", [
          addr2.address,
          addr5.address,
        ]);
    ...
    it("updates isOwner status for previous address", async () => {
      expect(await MultiSigWalletV3Contract.isOwner(addr2.address)).to.equal(
        true
      );
    ...
    it("does not allow other wallets to directly call the addOwner method", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(owner).replaceOwner(
          addr4.address,
          addr6.address
        )
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "NotMultiSig");
  ...
  describe("Change Requirements", async () => {
    ...
    beforeEach(async () => {
      const changeRequirementData =
        MultiSigWalletV3ContractInterface.encodeFunctionData(
          "changeRequirement",
          [newRequirement]
        );
    ...
    it("does not allow other wallets to directly call the changeRequirement method", async () => {
      await expect(
        MultiSigWalletV3Contract.connect(owner).changeRequirement(2)
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "NotMultiSig");
    });
    ...
    it("does not allow requirement to increase above the number of owners", async () => {
      ...
      await expect(
        MultiSigWalletV3Contract.connect(addr1).executeTransaction(nextTxIndex)
      ).to.be.revertedWithCustomError(MultiSigWalletV3Contract, "TxFailure");
    });
  });
});
